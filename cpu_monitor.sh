...
function main() {
  ...
  local cpu_str="cpu_total:$cpu_total%;"
  ...
  for pid in ${pids[*]}; do
    if [[ "$pid" != "" ]]; then
      ...
      cpu_str=$cpu_str"pname:$proc_name,pid:$pid,cpu:$proc_cpu%;"
    fi
    i=$((i+1))
  done
  
  cpu_str=$(echo $cpu_str | sed 's\;$\\g')

  if (( $cpu_total <= 80 )); then
    printf "%s %-10s %-20s\t%s\n" "$(date +"%F %T:%3N")" $log_info ${FUNCNAME[0]} "$cpu_str" >> $first_log
  else
    printf "%s %-10s %-20s\t%s\n" "$(date +"%F %T:%3N")" $log_error ${FUNCNAME[0]} "$cpu_str" >> $first_log
  fi
