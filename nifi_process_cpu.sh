#!/bin/sh
# awk '{hour[$1"_"substr($2,1,2)]+=$11; cnt[$1"_"substr($2,1,2)]++} END{for(i in hour) {printf "%s %.2f\n",i,hour[i]/cnt[i]}}'| sort -nk 1
...
while (true); do
  ...
  echo "$date_str $(top -p $pid -bn 1|tail -2|head -1)" >> $first_log
